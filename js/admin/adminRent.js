...
function viewSize()
{
	...
	var countStokProduct = 0;
	var countProductInRent = 0;
	
	$.each(JSON_stock_products, function(j, stock){
		if(stock.idBranch == idBranch && stock.idProduct == idProduct && stock.idColor == idColor){	countStokProduct = parseInt(stock.iStock);}
	});

	$.each(list_schedule, function(k, schedule){
		if(parseInt(schedule.idTypeSchedule) == 1 && schedule.idBranch == idBranch && schedule.idProduct == idProduct && schedule.idColor == idColor)
		{
			var dateEvent = dateIsBetweenToDates(schedule.vDateEvent);
			var dateDelivery = dateIsBetweenToDates(schedule.vDateDelivery);
			if (dateEvent || dateDelivery) {
	...
	if(countStokProduct > countProductInRent)
	{
		$.each(JSON_stock_products, function(j, stock){
...
function valPrice()
{
	...
	if($(".hdn_price_rent").val() == ""){ subtotal = 0;}
	else {subtotal = parseInt($(".hdn_price_rent").val());}
